...
export default class extends Controller {
  ...
  focusSearch() {
    const search = this.element.querySelector(".fl-search .fl-input, input[type='search']")
    if (search) {
      search.focus()
      search.select()
    }
  ...
  switchTabByIndex(index) {
    ...
    const tabs = this.tabsTarget.querySelectorAll(".fl-tab")
  ...
  toggleTheme() {
    const themeController = this.application.getControllerForElementAndIdentifier(
      document.documentElement, "faultline--theme"
    )
    if (themeController) {
      ...
      localStorage.setItem("faultline-theme", isDark ? "light" : "dark")
    }
  }
  ...
  navigateToPrev() {
    const prevLink = document.querySelector(".fl-nav-link[href*='logged_exceptions']")
    if (prevLink && prevLink.querySelector(".fl-nav-link-label")?.textContent === "Previous") prevLink.click()
  }

  navigateToNext() {
    const links = document.querySelectorAll(".fl-nav-link[href*='logged_exceptions']")
    const nextLink = Array.from(links).find(l => l.querySelector(".fl-nav-link-label")?.textContent === "Next")
    if (nextLink) nextLink.click()
  }

  copyId(event) {
    const id = event.currentTarget.dataset.faultlineIdValue
    if (id) this.copyToClipboard(id, event.currentTarget)
  }

  copyMessage(event) {
    const msg = event.currentTarget.dataset.faultlineMessageValue
    if (msg) this.copyToClipboard(msg, event.currentTarget)
  }

  copyBacktrace(event) {
    const bt = event.currentTarget.dataset.faultlineBacktraceValue
