  ...
  class Logger
    ...
    class << self
      ...
      def write_to_output(entry)
        ...
        line = "#{color}[Faultline]#{reset} #{ts} #{level.to_s.upcase.ljust(5)}"
      ...
      def write_to_file(entry)
        file = Faultline.configuration.log_file
        ...
        Rails.logger.error("[Faultline] Failed to write to log file: #{e.message}")
      end
