...
export default class extends Controller {
  ...
  navigateToPrev() {
    ...
    if (prevLink && prevLink.querySelector(".fl-nav-link-label")?.textContent === "Previous") {
      prevLink.click()
    }
  }
  ...
  navigateToNext() {
    ...
    const nextLink = Array.from(links).find(link =>
      link.querySelector(".fl-nav-link-label")?.textContent === "Next"
    )
  ...
  copyMessage(event) {
    const message = event.currentTarget.dataset.faultlineMessageValue
    if (message) this.copyToClipboard(message, event.currentTarget)
  }

  copyBacktrace(event) {
    const backtrace = event.currentTarget.dataset.faultlineBacktraceValue
